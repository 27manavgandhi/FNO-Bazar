...
function OiTracker(props) {
  const { oiTrackerOptions, setOiTrackerOptions, theme } = props;
  const [toggleButton, setToggleButton] = useState(false);
  const [componentLoading, setComponentLoading] = useState(true);
  const [oiTrackerData, setOiTrackerData] = useState([]);
  ...
  //   const oiTrackerRef = ref(rtdb, `recent data/`);
  //   const fetchOichanger = async () => {
  //     const callsSnapshot = await get(oiTrackerRef);
  //     const Data = callsSnapshot.val();
  //     console.log(Data);
  //     setOiTrackerData(Data);
  //   };
  //   fetchOichanger();
  // }, []);
  ...
  const loadOptionsFromLocalStorage = () => {
    const savedOptions = JSON.parse(localStorage.getItem("OITrackerOptions"));
    if (savedOptions) {
      setOiTrackerOptions(savedOptions);
    }
  };
  ...
  const updateOptionChainOptions = (updatedOptions) => {
    setOiTrackerOptions(updatedOptions);
    localStorage.setItem("OITrackerOptions", JSON.stringify(updatedOptions));
  };
  ...
  useEffect(() => {
    const date =
      oiTrackerOptions.index === "NIFTY"
        ? indexWiseExpiry.nifty[0]
        : indexWiseExpiry.banknifty[0];
    setExpiry(date);
  }, [oiTrackerOptions, indexWiseExpiry]);
  //-----strike price calculation------
  useEffect(() => {
    const index =
      oiTrackerOptions.index === "NIFTY" ? "NIFTY 50" : "NIFTY BANK";
    ...
    const unsubscribe = onValue(niftyLTPRef, (snapshot) => {
      ...
      if (niftyLTPData) {
        strikePriceArray = buildStrikePricesArray(
          niftyLTPData,
          2,
          oiTrackerOptions["index"] === "NIFTY" ? 50 : 100
        );
  ...
  useEffect(() => {
    ...
    let strikRef;
    const callorput = oiTrackerOptions["callorput"];
    const index = oiTrackerOptions["index"];
    const indices =
      oiTrackerOptions["index"] === "NIFTY" ? "NIFTY 50" : "NIFTY BANK";
    //Data Fetching code
    ...
    // tempData
  }, [oiTrackerOptions, expiry, timestampsArray]);
  const index = oiTrackerOptions["index"];
  console.log(oiTrackerData);
  if (oiTrackerData.length === 0) return <>loading...</>;
  // STYLE FOR MODAL
  const modalStyle = {
    position: "absolute",
    top: "50%",
            ...
            <div className=" subpage-dropdowns-container">
              ...
              <div className="dropdown-container">
                <select
                  ...
                  value={oiTrackerOptions.lotOrValue}
                >
              ...
              <div className="dropdown-container">
                <select
                  ...
                  value={oiTrackerOptions.numStrikePrice}
                >
          ...
          <tbody>
            <tr className="textbold">
              ...
              {/* {strikeArray.map((strike, i) => {
                const coi = (
                  oiTrackerData?.[oiTrackerOptions.callorput]?.[strike]?.OI -
                  oiTrackerData?.[oiTrackerOptions.callorput]?.[strike]
                    ?.Prev_Close
                ).toFixed(0);
            ...
            {timestampsArray.map((item, index) => {
              ...
              return (
                <tr key={item}>
                  ...
                  {strikeArray.map((strike, i) => {
                    const previousCoi = (
                      oiTrackerData[index - 1]?.[oiTrackerOptions.callorput]?.[
                        strike
                      ]?.OI -
                      oiTrackerData[index - 1]?.[oiTrackerOptions.callorput]?.[
                        strike
                    ...
                    const coi = (
                      oiTrackerData[index]?.[oiTrackerOptions.callorput]?.[
                        strike
                      ]?.OI -
                      oiTrackerData[index]?.[oiTrackerOptions.callorput]?.[
                        strike
                    ...
                    let color = "";
                    if (difference >= oiTrackerOptions.value) {
                      color = "green";
                    }
                    if (difference < 0) {
                      color = "red";
